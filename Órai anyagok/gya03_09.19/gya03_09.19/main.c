/******************************************************************************
 * Created:		19-09-2024 12h
 * Author :		Balassa Hodi
 * File name:	gyak03_09.19
******************************************************************************/
 /******************************************************************************
* Include files
******************************************************************************/
...
* Description:      
* Input:			baud -> kommunikacios sebesseg (p.202)
* Output:
* Notes:			AT90CAN32 p.183
******************************************************************************/
void uart0_init(uint16_t baud)
{
	...
	UCSR0C = (0<<UMSEL0) | (0<<UPM0) | (1<<USBS0) | (1<<UCSZ1) | (1<<UCSZ0); // igy lesz 1 stopbit a 2 helyett
	
	/* Enable receiver and transmitter */
	UCSR0B = (1<<RXEN0) | (1<<TXEN0) | (1<<RXCIE0); // igy a fogadas interruptos uzemmodba van allitva
}
...
ISR(USART0_RX_vect) { // UART interrupt
	char c = UDR0;
	if(c == 0x7f) { // 0x7f ->backspace ascii code-ja
		lcd_clear_display();
	} else lcd_write_char(c);
}